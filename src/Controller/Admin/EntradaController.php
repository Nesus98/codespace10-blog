...
class EntradaController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $em, EntradaRepository $entradaRepository, UsuarioRepository $usuarioRepository): Response
    {
        $entrada = new Entrada();
    ...
    {
        if ($this->isCsrfTokenValid('delete'.$entrada->getId(), $request->getPayload()->get('_token'))) {
